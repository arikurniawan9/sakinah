...
export function useProductTable() {
  ...
  const fetchProducts = useCallback(async (forceRefresh = false) => {
    try {
      ...
      // Tambahkan parameter untuk memaksa refresh cache jika diperlukan
      if (forceRefresh) {
        params.append('_', Date.now()); // Parameter cache-busting
      }
  ...
  // Fungsi untuk optimistic update penghapusan produk
  const removeProductsOptimistic = useCallback((idsToRemove) => {
    setProducts(prevProducts => prevProducts.filter(product => !idsToRemove.includes(product.id)));
    setTotalProducts(prev => Math.max(0, prev - idsToRemove.length));
  }, []);

  // Fungsi untuk optimistic update penambahan produk
  const addProductOptimistic = useCallback((newProduct) => {
    setProducts(prevProducts => [newProduct, ...prevProducts]);
    setTotalProducts(prev => prev + 1);
  }, []);

  // Fungsi untuk optimistic update pengubahan produk
  const updateProductOptimistic = useCallback((updatedProduct) => {
