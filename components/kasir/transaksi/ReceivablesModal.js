// components/kasir/transaksi/ReceivablesModal.js
'use client';
...
const ReceivablesModal = ({ isOpen, onClose, memberId, darkMode }) => {
  ...
  const fetchReceivables = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/receivables?memberId=${memberId}&status=UNPAID,PARTIALLY_PAID`);
      if (response.ok) {
  ...
  return (
    ...
    >
      <div
        className={`relative rounded-xl shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white dark:bg-gray-800 border ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}
        ...
        <div className="p-6">
          {loading ? (
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="flex items-center">
                          ...
                          <span className="font-semibold">Invoice: {receivable.sale?.invoiceNumber || receivable.id}</span>
                        </div>
                        <p className="text-sm mt-1">Tanggal: {formatDateTime(receivable.sale?.date || receivable.createdAt)}</p>
            ...
            <div className="text-center py-8">
              ...
              <p className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>Member ini tidak memiliki hutang yang belum lunas</p>
            </div>
          )}
        </div>
      </div>
