          ...
          <tr>
            <th scope="col" className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider w-12 ${
              ...
              <input
                ...
                checked={attendants.length > 0 && selectedRows.length === attendants.length}
        ...
        <tbody className={`${darkMode ? 'divide-gray-700 bg-gray-800' : 'bg-gray-50'} divide-y ${
          ...
          {attendants.length === 0 ? (
            ...
            attendants.map((attendant) => (
              <tr key={attendant.id} className={`${darkMode ? 'hover:bg-gray-700' : 'hover:bg-pastel-purple-50'}`}>
                <td className="px-6 py-4 whitespace-nowrap">
                  <input
                    type="checkbox"
                    checked={selectedRows.includes(attendant.id)}
                    onChange={() => {
                      console.log('Individual checkbox onChange fired for ID:', attendant.id);
                      handleSelectRow(attendant.id);
                    }}
                    className={`h-4 w-4 rounded ${
                      darkMode 
                        ? 'text-purple-600 bg-gray-700 border-gray-600' 
