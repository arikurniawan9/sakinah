...
export const useUserForm = (refreshData, options = {}) => {
  ...
  const validateForm = () => {
    ...
    if (isManagerContext && !isWarehouseContext && formData.role !== ROLES.MANAGER && formData.role !== ROLES.WAREHOUSE && !formData.storeId) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isManagerContext) {
        ...
        apiEndpoint = defaultApiEndpoint || '/api/warehouse/users';
      }

      const url = editingUser ? `${apiEndpoint}/${editingUser.id}` : apiEndpoint;
      const method = editingUser ? 'PUT' : 'POST';

      // Siapkan data user dengan role yang disesuaikan
      const adjustedFormData = { ...formData };

      // Jika dalam konteks warehouse dan role belum ditentukan, gunakan role WAREHOUSE
      if (isWarehouseContext && !adjustedFormData.role) {
        adjustedFormData.role = ROLES.WAREHOUSE;
      }

      const userData = {
        name: adjustedFormData.name.trim(),
        username: adjustedFormData.username.trim(),
        employeeNumber: adjustedFormData.employeeNumber ? adjustedFormData.employeeNumber.trim() : null,
        code: adjustedFormData.code ? adjustedFormData.code.trim() : null,
        role: adjustedFormData.role,
        address: adjustedFormData.address,
