...
export async function logActivity(userId, action, entity, entityId, description, oldValue = null, newValue = null, storeId = null) {
  try {
    // Jika storeId tidak disediakan dan ini adalah entitas yang terkait dengan warehouse,
    // kita perlu mendapatkan storeId dari warehouse store
    let finalStoreId = storeId;

    if (!finalStoreId) {
      // Untuk entitas user warehouse, kita perlu mendapatkan storeId dari data newValue
      if (newValue && typeof newValue === 'object' && newValue.storeId) {
        finalStoreId = newValue.storeId;
      } else if (newValue && typeof newValue === 'string') {
        try {
          const parsedNewValue = JSON.parse(newValue);
          if (parsedNewValue.storeId) {
            finalStoreId = parsedNewValue.storeId;
          }
        } catch (e) {
          // Jika parsing gagal, lanjutkan tanpa storeId
        }
      }
    }

    // Jika tetap tidak ada storeId, kita perlu mendapatkan storeId default untuk warehouse
    if (!finalStoreId) {
      // Coba dapatkan store warehouse default
      const warehouseStore = await prisma.store.findFirst({
        where: {
          code: 'GM001' // Kode default untuk warehouse store sesuai konstanta
        }
      });

      if (warehouseStore) {
        finalStoreId = warehouseStore.id;
      } else {
        // Jika tidak ada warehouse store, cari store pertama
        const firstStore = await prisma.store.findFirst();
        if (firstStore) {
          finalStoreId = firstStore.id;
        } else {
          // Jika tidak ada store sama sekali, kita bisa membuat store default untuk aktivitas umum
          // Buat store default untuk aktivitas yang tidak terkait dengan store tertentu
          try {
            const defaultStore = await prisma.store.create({
              data: {
                name: 'System Default Store',
                code: 'GM001',
                description: 'Store default untuk aktivitas sistem'
              }
            });
            finalStoreId = defaultStore.id;
          } catch (createError) {
            // Jika store dengan kode GM001 sudah ada, ambil saja
            if (createError.code === 'P2002') { // Unique constraint violation
              ...
              if (existingStore) {
                ...
                throw new Error('Tidak ditemukan toko untuk mencatat aktivitas');
              }
            } else {
              throw new Error('Tidak ditemukan toko untuk mencatat aktivitas');
            }
