      ...
      try {
        ...
        if (result && result.status >= 200 && result.status < 300) {
          await logAudit({
            userId,
            action,
            entity: tableName || 'UNKNOWN_ENTITY', // Added entity
            tableName,
            ipAddress,
            userAgent,
            storeId,
...
export async function logMemberCreation(userId, member, storeId, ipAddress, userAgent) {
  await logAudit({
    ...
    newValue: {
      ...
      // discount: member.discount, // Discount might not be present in member creation directly
    },
    ipAddress,
    userAgent,
    storeId,
...
export async function logStoreCreation(userId, store, ipAddress, userAgent) {
  await logAudit({
    userId,
    action: AUDIT_ACTIONS.USER_CREATE, // consider creating AUDIT_ACTIONS.STORE_CREATE
    entity: 'Store', // Added entity
    tableName: 'Store',
    recordId: store.id,
    newValue: {
      name: store.name,
...
export async function logStoreUpdate(userId, oldStore, newStore, ipAddress, userAgent) {
  await logAudit({
    userId,
    action: AUDIT_ACTIONS.USER_UPDATE, // consider creating AUDIT_ACTIONS.STORE_UPDATE
    entity: 'Store', // Added entity
    tableName: 'Store',
...
export async function logStoreDeactivation(userId, store, ipAddress, userAgent) {
  await logAudit({
    userId,
    action: AUDIT_ACTIONS.USER_UPDATE, // consider creating AUDIT_ACTIONS.STORE_DEACTIVATE
    entity: 'Store', // Added entity
    tableName: 'Store',
    recordId: store.id,
...
export async function logUserCreation(userId, user, ipAddress, userAgent, storeId) {
  await logAudit({
    ...
    entity: 'User', // Added entity
    tableName: 'User',
    recordId: user.id,
    newValue: {
      name: user.name,
      username: user.username,
      // email: user.email, // email is not directly on user model, based on prisma/schema.prisma
      role: user.role,
      status: user.status,
    },
    ipAddress,
...
export async function logCreate(userId, entityName, entityData, ipAddress, userAgent, storeId) {
  ...
  await logAudit({
    ...
    entity: entityName, // Added entity
...
export async function logWarehouseDistribution(userId, distributionData, ipAddress, userAgent, storeId) {
  await logAudit({
    ...
    entity: 'WarehouseDistribution', // Added entity here
...
export async function logWarehousePurchase(userId, purchaseData, ipAddress, userAgent, storeId) {
  await logAudit({
    ...
    entity: 'WarehousePurchase', // Added entity
...
export async function logWarehouseStockAdjustment(userId, adjustmentData, ipAddress, userAgent, storeId) {
  await logAudit({
    ...
    entity: 'WarehouseStockAdjustment', // Added entity
    tableName: 'WarehouseProduct', // Adjust tableName if 'WarehouseStockAdjustment' is a model
