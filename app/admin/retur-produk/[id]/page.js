...
import { PackageX, User, Package, Calendar, Store, AlertCircle, CheckCircle, XCircle, Clock, Edit3, ArrowLeft } from 'lucide-react';
...
export default function ReturnProductDetailPage() {
  ...
  const breadcrumbItems = [
    ...
    { title: `Detail #${params.id}`, href: `/admin/retur-produk/${params.id}` }
  ];
  ...
  const handleApprove = async () => {
    if (!returnData) {
      alert('Data retur produk tidak ditemukan');
      return;
    }

    const confirmed = confirm('Apakah Anda yakin ingin menyetujui retur produk ini?');
    if (!confirmed) return;

    try {
      // Dapatkan ID user yang sedang login - dalam implementasi nyata ini akan diambil dari auth context
      ...
      if (result.success) {
        alert('Retur produk berhasil disetujui');
        // Refresh the page to show updated status
        router.refresh();
      } else {
        console.error('Failed to approve return:', result.message);
        alert(`Gagal menyetujui retur: ${result.message}`);
      }
    } catch (error) {
  ...
  const handleReject = async () => {
    ...
    const confirmed = confirm('Apakah Anda yakin ingin menolak retur produk ini?');
    if (!confirmed) return;
    ...
    try {
      ...
      if (result.success) {
        alert('Retur produk berhasil ditolak');
        // Refresh the page to show updated status
        router.refresh();
      } else {
        console.error('Failed to reject return:', result.message);
        alert(`Gagal menolak retur: ${result.message}`);
      }
    } catch (error) {
    ...
    <ProtectedRoute requiredRole="ADMIN">
      <div className={`w-full px-4 sm:px-6 lg:px-8 py-8 ${userTheme.darkMode ? 'dark' : ''}`}>
        ...
        <div className="mb-8">
          <div className="flex items-center space-x-4">
            <Button 
              variant="outline" 
              size="icon"
              onClick={() => router.push('/admin/retur-produk')}
              className="p-2"
            >
            ...
            <div>
              ...
              <p className={`mt-2 ${userTheme.darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                Informasi lengkap tentang permintaan retur produk
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Sidebar with Actions and Related Info */}
            ...
            {returnData.status === 'PENDING' && (
              <Card className={userTheme.darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}>
                <CardHeader>
                  ...
                  <CardDescription>
                    Proses permintaan retur produk ini
                  </CardDescription>
                </CardHeader>
